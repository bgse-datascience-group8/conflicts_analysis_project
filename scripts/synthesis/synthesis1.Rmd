...
plot(least_sqrs_beta.residuals, pch = 19, main = 'Boston: 1-day Lag Model Residuals')
```
...
```{r, echo=FALSE, warnings=FALSE, messages=FALSE }
# 21 days selected for best effect-dropoff
ndays <- 14
...
matplot(coeffs, type='l', xlab = "nth-Day Lag", ylab = 'Coefficient', main = 'Auto correlation: effect by day')

matplot(lag_mean_eff, type='l', xlab = "nth-Day Lag", ylab = 'Average Coefficient', main = 'Auto correlation: Mean effect by day', ylim = c(0.25,0.75))
...
results <- space.joint(as.matrix(events_by_date_city),lam1=100)
...
for (i in 1:nrow(cities)) {
  ...
  day_avgs_same_region <- rowMeans(events_by_date_city.v2[,same_region_cities])
  day_avgs_diff_regions <- rowMeans(events_by_date_city.v2[,setdiff(colnames(events_by_date_city), c(same_region_cities,city$city))])
  
  model <- glm(events_by_date_city.v2[,city$city] ~ day_avgs_same_region + day_avgs_diff_regions, family='poisson')
  summary(model)
}
```