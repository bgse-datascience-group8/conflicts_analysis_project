...
png(paste0(images_dir,'analysis_boston_ex1.png'), bg = "transparent", width=600, height=600, units='px')
plot
dev.off()
...
png(paste0(images_dir,'analysis_network_basic_lasso.png'), bg = "transparent", width=1000, height=1000, units='px')
...
png(paste0(images_dir,'analysis_network_basic_lasso_hist.png'), bg = "transparent", width=600, height=400, units='px')
...
map <- map + 
  ...
  geom_text(data = cities, 
             aes(x = as.numeric(city.long), y = as.numeric(city.lat), label = city.name), colour = 'royalblue4', vjust = 1, hjust = .8) + theme(panel.background = element_blank())

png(paste0(images_dir,'analysis_network_basic_lasso_map.png'), bg = "transparent", width=1000, height=600, units='px')
map
dev.off()
```
...
png(paste0(images_dir,'analysis_network_space.png'), bg = "transparent", width=1000, height=1000, units='px')
...
png(paste0(images_dir,'analysis_network_space_hist.png'), bg = "transparent", width=600, height=400, units='px')
hist(V(g)$degree, 30, main='Degree of vertices', xlab='Degree')
dev.off()
```
...
master.table.2 <- master.table[,c('log_num_conflicts','lag.log_num_conflicts','log_population','log_population_sq', 'isWashingtonDC','weekend')]
master.table.2[is.na(master.table.2)] <- 0
#head(master.table.2)
...
mrs <- mean(r_squareds)
mmse <- mean(MSEs)
mat <- matrix(c(r_squareds, mrs, MSEs, mmse), ncol=6, nrow=2, byrow = TRUE)
colnames <- c(unlist(lapply('Test ', paste0, 1:5)), 'Average')
rownames <- c('R-squared', 'Mean Squared Error')
