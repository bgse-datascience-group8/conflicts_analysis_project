...
summary(m)
#[ADD ME TO APP]
ggplot(data, aes(x=log(population), y = log(city_mean), color=city)) + 
  geom_point() + 
  geom_line(aes(y=fitted.values(m)), color='darkgrey') +
...
data2 <- data[,c('log_num_conflicts', 'city', 'datef')]
...
colnames(data2) <- c('datef','city','log_num_conflicts','lag.log_num_conflicts')
#final$city <- as.character(final$city)
#data2$city <- toString(data2$city)
data3 <- merge(data2, final, by=c('city','datef'))

master.table <- merge(data3, city_data, by='city')
#head(master.table <- unique(master.table)

master.table.2 <- master.table[,c('log_num_conflicts','lag.log_num_conflicts','same_region','region.1','region.2','region.3','region.4','region.5','log_population','log_population_sq', 'isWashingtonDC')]
master.table.2$city_mean <- as.numeric(master.table.2$city_mean)
