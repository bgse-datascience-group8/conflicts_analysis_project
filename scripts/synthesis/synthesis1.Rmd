...
subsets <- seq(1,length(cities),1)
subset_size <- 6
subsets <- split(subsets, ceiling(seq_along(subsets)/(length(cities)/subset_size)))

plots <- list()

for (idcs in 1:subset_size) {
  current_cities <- cities[unlist(subsets[idcs])]
  current_data <- subset(data, city %in% current_cities)
  plot <- ggplot(data = current_data, aes(x = datef, y = num_conflicts, color = city)) + 
...
for (idcs in 1:subset_size) {
  current_cities <- cities[unlist(subsets[idcs])]
  current_data <- subset(data, city %in% current_cities)
  plot <- ggplot(data = current_data, aes(x = datef, y = std_num_conflicts, color = city)) + 
    geom_smooth(method="loess", se=FALSE) + 
    theme(legend.position = "none")
...
y <- epsilon.t.master[,1]
X <- epsilon.t.master[,2:14]
p <- ncol(X)
n <- length(y)
model <- lm(y ~ 0 + X)
rss <- my.rss(y,X)
# compare with: sum(residuals(model)**2)
tss <- my.tss(y,X)
ssm <- my.ssm(y,X)
# (SSM/DFM) / (SSE/DFE)
f.stat <- ((ssm)/(p))/(rss/(n - p))

# R^2 = MSE/MST - 1
# MSE = SSE/DFE = SSE/(n-p) and MST = SST/DFT = SST/(n-1)
mse <- rss/(n-p)
mst <- tss/(n-1)
r.squared <- mse/mst - 1
