...
```{r}
dimnames(P.hat) <- list(cities,cities)
# to analyze boston we want to regress only the columnts of events_by_date_city which space has selected
A.hat.boston <- as.matrix(events_by_date_city) %*% diag(A.hat[,'Boston, MA'])
colnames(A.hat.boston) <- cities
# Regress boston on all other cities
A.hat.boston.df <- as.data.frame(A.hat.boston)
library(R.utils)
A.hat.boston.df <- A.hat.boston.df[, colSums(isZero(A.hat.boston.df)) != nrow(A.hat.boston.df)]
A.hat.boston.df <- cbind(A.hat.boston.df, boston = events_by_date_city[,'Boston, MA'])
head(A.hat.boston.df)
model <- lm(`boston` ~ ., A.hat.boston.df)
```
